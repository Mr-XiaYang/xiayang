...
let regex = RegExp(`^([Dd][Aa][Tt][Aa]|[Aa][Rr][Ii][Aa]|x)-.*$`);

const verifyCache: Record<string, boolean> = {};

function isValidProp(prop: string, expandProps: Record<string, boolean> = {}): boolean {
  if (verifyCache[prop] == null) {
    verifyCache[prop] = (
      expandProps[prop] || props[prop] || regex.test(prop)
    ) && (
      prop.charCodeAt(0) === 111 && prop.charCodeAt(1) === 110 && prop.charCodeAt(2) < 91
    );
  }
  return verifyCache[prop];
}
